...
class Extractor(object):
    ...
    def run_on_image(self, image, confidence_threshold, path):
        ...
        if "instances" in predictions:
            ...
            predictions["instances"] = instances.to(self.cpu_device)
...
def main(args):
    ...
    for image_file in tqdm(os.listdir(BASE_DIR), desc="Extracting"):
        ...
        if os.path.isfile(f"swintextspotter/{image_id}.npy"):
            continue
        img = read_image(os.path.join(BASE_DIR, image_file), format="BGR")
        predictions = extractor.run_on_image(img, 0.3, os.path.join(BASE_DIR, image_file))
        ...
        np.save(f"swintextspotter/{image_id}.npy", features, allow_pickle=True)
